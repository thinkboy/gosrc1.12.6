...
// 把fd注册到epoll里并关联一个pollDesc结构对象
//go:linkname poll_runtime_pollOpen internal/poll.runtime_pollOpen
func poll_runtime_pollOpen(fd uintptr) (*pollDesc, int) {
	pd := pollcache.alloc() // 创建一个pollDesc对象
	lock(&pd.lock)
	...
	var errno int32
	errno = netpollopen(fd, pd) // 注册fd到内核epoll里
	return pd, int(errno)
}
...
// 检查是否异常socket，如果socket关闭了或者超时了，则返回非0，正常返回0
...
// 该方法在netpollblock中进入休眠时掉用
// 如果gpp被标记为pdWait状态，说明要进入等待，然后把gpp指向当前执行的G（参数gp是当前执行的G），然后返回true
