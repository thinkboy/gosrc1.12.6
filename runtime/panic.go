...
func deferproc(siz int32, fn *funcval) { // arguments of fn follow fn
	...
	sp := getcallersp() // 调用defer函数的SP寄存器
	argp := uintptr(unsafe.Pointer(&fn)) + unsafe.Sizeof(fn)
	callerpc := getcallerpc() //调用defer函数的PC寄存器

	d := newdefer(siz) // 创建一个维护defer函数的结构
	if d._panic != nil {
	...
	default:
		memmove(deferArgs(d), unsafe.Pointer(argp), uintptr(siz)) // 调用关联的参数放在_defer栈帧的后面(deferArgs(d)是把指针移动到d结构体的末尾)
	}
...
//
// 从P本地或者全局_defer对象缓存池里取一个出来，如果都没有就从堆里new一个
//go:nosplit
func newdefer(siz int32) *_defer {
	...
	if sc < uintptr(len(p{}.deferpool)) {
		pp := gp.m.p.ptr()
		if len(pp.deferpool[sc]) == 0 && sched.deferpool[sc] != nil { // 如果P本地缓存池里没有，但全局缓存池里有，则取一半到P本地缓存池
		...
		if n := len(pp.deferpool[sc]); n > 0 { // 如果对应sizeclass的缓存池里有，则从队尾取一个出来
			d = pp.deferpool[sc][n-1]
			pp.deferpool[sc][n-1] = nil
			pp.deferpool[sc] = pp.deferpool[sc][:n-1]
		}
	}
	if d == nil { // 到这里说明本地P跟全局缓存池里都没有，则从系统分配一个
		// Allocate new defer+args.
		systemstack(func() {
			total := roundupsize(totaldefersize(uintptr(siz)))
			d = (*_defer)(mallocgc(total, deferType, true)) // 从系统分配一个对象
		})
		if debugCachedWork {
	...
	d.siz = siz
	d.link = gp._defer // _defer是一个链表结构
	gp._defer = d // 把_defer放到当前G的_defer链表头部
	return d
}
...
func freedefer(d *_defer) {
	...
	pp := getg().m.p.ptr()
	if len(pp.deferpool[sc]) == cap(pp.deferpool[sc]) {// 如果deferpool对应sizeclass里已经满了，则把一半放到全局缓存池里。（deferpool设计里是不允许内存扩张的，所以cap是固定的大小）
	...
	pp.deferpool[sc] = append(pp.deferpool[sc], d) // 放到P本地缓存池里
}
...
func deferreturn(arg0 uintptr) {
	...
	default:
		memmove(unsafe.Pointer(&arg0), deferArgs(d), uintptr(d.siz)) // 取出来参数
	}
	fn := d.fn
	d.fn = nil
	gp._defer = d.link // 让gp._defer指向下一个元素，因此是从链表头里面取出来_defer
	freedefer(d) // 把_defer结构归还给缓存池
	jmpdefer(fn, uintptr(unsafe.Pointer(&arg0)))// 该方法由汇编实现,在实现里会再次跳转到deferreturn函数里，实现了一个循环调用，直到gp._defer里没有可执行的函数
}
...
func gopanic(e interface{}) {
	...
	p.arg = e
	p.link = gp._panic
	gp._panic = (*_panic)(noescape(unsafe.Pointer(&p))) // 定义一个新的_panic结构放到当前G的_panic链表里

	atomic.Xadd(&runningPanicDefers, 1)

	// 遍历所有defer的函数，并执行
	// 如果有recover，则恢复到defer的函数栈继续执行，否则退出进程
	for {
		d := gp._defer // 如果有defer待执行，则从defer链表头部取一个
		if d == nil {
			break
		}
		...
		if d.started { // 如果已经被早期的panic或者Goexit开始执行了当前的defer了，则跳过处理
			if d._panic != nil {
				d._panic.aborted = true
			}
			d._panic = nil
		...
		freedefer(d)
		if p.recovered {// 如果有recover函数则执行recover函数
			atomic.Xadd(&runningPanicDefers, -1)
			...
			gp.sigcode0 = uintptr(sp) // 记录_defer的栈地址寄存器
			gp.sigcode1 = pc
			mcall(recovery) // 调用recovery方法恢复寄存器到_defer栈上，此函数不在return，将继续执行
	...
	fatalpanic(gp._panic) // 进程退出 // should not return
...
// 给当前G的_pannic里设置recover标记位，在gopanic里实现真正的recover操作
//go:nosplit
