...
func stackpoolalloc(order uint8) gclinkptr {
	...
	if s == nil {
		...
		s = mheap_.allocManual(_StackCacheSize>>_PageShift, &memstats.stacks_inuse) // 从堆里分配个mspan
		if s == nil {
			throw("out of memory")
		}
		if s.allocCount != 0 {
		...
		osStackAlloc(s) // linux下该方法为空，不做任何操作
		s.elemsize = _FixedStack << order
		for i := uintptr(0); i < _StackCacheSize; i += s.elemsize { // 拆分成list
...
func stackalloc(n uint32) stack {
	...
	thisg := getg()
	if thisg != thisg.m.g0 { // 栈分配不能发生在调度器用的栈上
		throw("stackalloc not on scheduler stack")
	}
	if n&(n-1) != 0 { // 栈大小必须是2的倍数
	...
	// a dedicated span.
	// 小的栈空间(<32KB)通过一个固定大小的free-list分配器分配。
	// 如果我们需要一个大的栈空间(>=32KB)，我们求助于专用的span。
	var v unsafe.Pointer
	if n < _FixedStack<<_NumStackOrders && n < _StackCacheSize { //如果n<32KB，即_FixedStack==2048  _NumStackOrders==4  _StackCacheSize==32768
		order := uint8(0)
		n2 := n
		for n2 > _FixedStack {
			order++
...
func stackfree(stk stack) {
	...
	if n < _FixedStack<<_NumStackOrders && n < _StackCacheSize { // 如果是小栈(<32KB),则放到mcache本地stack缓存里或者放到全局小栈缓存池
		...
		if gcphase == _GCoff { // 如果正在GC，则直接返还给mheap
...
func copystack(gp *g, newsize uintptr, sync bool) {
	...
	memmove(unsafe.Pointer(new.hi-ncopy), unsafe.Pointer(old.hi-ncopy), ncopy) // copy老的栈到新栈上
...
//
// 分配一个大的栈，把老栈迁移到新栈上
// 在runtime·morestack中调用(morestack是汇编实现，位置在runtime/asm_amd64.s)
...
func newstack() {
	...
	// value now and below.
	// 栈扩张之前判断当前G是否被标记了抢占,如果 gp.stackguard0 == stackPreempt 则说明是抢占触发的栈扩张
	...
	//如果判断可以抢占, 则继续判断是否GC引起的, 如果是，则对G的栈空间执行标记处理(扫描根对象)然后继续运行,
	...
	if preempt {
		...
		casgstatus(gp, _Gwaiting, _Grunning)
		// 停止当前运行状态的G,最后放到全局runq里,释放M,表示被抢占了释放出来资源继续调度循环
		// 进入schedule循环,不在返回
	...
	copystack(gp, newsize, true) // copy G的栈到新栈上
	if stackDebug >= 1 {
		print("stack grow done\n")
	}
	casgstatus(gp, _Gcopystack, _Grunning)
	gogo(&gp.sched) // 栈扩张完成了，继续执行
}
